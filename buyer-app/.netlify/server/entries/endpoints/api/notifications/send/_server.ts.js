...
async function sendWhatsApp(to, template, vars = {}) {
  if (!WHATSAPP_TOKEN || !WHATSAPP_PHONE_ID) {
    console.log("[notify:whatsapp:stub]", { to, template, vars });
    return { ok: true, stub: true };
  }
  const components = Object.keys(vars).length ? [{ type: "body", parameters: Object.values(vars).map((v) => ({ type: "text", text: String(v) })) }] : [];
  const payload = {
