...
async function decrementStock(product_id, qty) {
...
const POST = async ({ request }) => {
  ...
  if (!SUPABASE_URL || !SERVICE_KEY) {
    ...
    const total2 = unit * qty;
    const fakeId = `stub-${Date.now()}`;
    console.log("[order:stub] created", { id: fakeId, total: total2, delivery_date });
    return new Response(JSON.stringify({ id: fakeId, total_naira: total2 }), { headers: { "content-type": "application/json" } });
  }
  ...
  try {
    await decrementStock(product_id, qty);
  } catch {
    await supabase.from("products").update({ stock_qty: prod.stock_qty - qty }).eq("id", product_id);
  }
